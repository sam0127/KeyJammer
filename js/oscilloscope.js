export const oscilloscopeInit = (canvasOscElement, canvasSpectElement, source) => {
    const atNode = (first, tolerance = 0) => {
        return first >= 128 - tolerance && first <= 128 + tolerance;
    };
    const getStabilizedWaveform = (size) => {
        let waveformSnapshot = new Uint8Array(size);
        var greatestSlope = 0;
        var greatestSlopes = [];
        ...
        if (greatestSlopes.length === 0) {
            ...
            var invert = (dataArrayOsc[middleIndex]
                - dataArrayOsc[middleIndex + 3]) > 0;
    ...
    const drawOsc = () => {
        ...
        const waveformArray = getStabilizedWaveform(bufferLength / 4);
    ...
    const drawSpect = () => {
        ...
        const spectArray = trimSpectrograph();
